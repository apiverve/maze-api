...
class MazeAPIClientError(Exception):
...
class ValidationError(MazeAPIClientError):
...
class MazeAPIClient:
    ...
    VALIDATION_RULES = {"width": {"type": "integer", "required": False, "min": 5, "max": 50, "default": 15}, "height": {"type": "integer", "required": False, "min": 5, "max": 50, "default": 15}, "difficulty": {"type": "string", "required": False, "default": "medium"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the MazeAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises MazeAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/maze'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[MazeAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns MazeGeneratorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises MazeAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise MazeAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise MazeAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise MazeAPIClientError(f"Request failed: {str(e)}")
